...
{
   int n;cin>>n;
   int a[n][3],xSum=0,ySum=0,zSum=0;
   REP(i,n) REP(j,3){cin>>a[i][j]; if(j%3==0) xSum+=a[i][j]; else if(j%3==1) ySum+=a[i][j]; else xSum+=a[i][j];}
   if(!xSum && !ySum && !zSum) cout<<"YES"; else cout<<"NO";
}
