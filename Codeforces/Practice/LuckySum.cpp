...
LLI convertToNumeric(string input)
{
  LLI ans=0;
  LLI x =1;
  REP(i,input.length())
  {
    ans += (input[input.length() -i -1]-'0') * x;
    x *= 10;
  }
  return ans;
}
...
LLI next(LLI l)
{
  ...
  while(i <=digits(l)+1 && !flaga)
  {
      string x="",y="";
      REP(j,i) {x+='4'; y+='7';}
      if(l <= convertToNumeric(x)) {flaga=1;m=x;break;}
      while(x!=y)
      {
          int j=i-1;
          while(x[j] != '4')
