...
public:
    ...
    Graph(int n){
        ...
        this->nodes = n;
        this->adjMat = new set<int>[n];
        this->visited = new bool[n];
        this -> distances = new int[n];
        REP(i,n) this -> distances[i] =0;
    }

    void addEdge(int u,int v){

        this->adjMat[u].insert(v);
    }
    void bfs(int s){
    ...
    while (!q.empty()) {
        ...
        for(it=adjMat[currentNode].begin();it!=adjMat[currentNode].end();it++){
            ...
            if(!visited[*it]){
                if(distances[*it]==-1){
                    distances[*it]=distances[currentNode]+6;
                    q.push(*it);
                }
                    ...
                    distances[*it]=min(distances[*it],distances[currentNode]+6);
                }
